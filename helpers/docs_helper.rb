  ...
  def link_to_editable_version
    ...
    repo = 
      if path.start_with?('doc/')
        'bundler/bundler'
      elsif path =~ %r{\Av\d+\.\d+/man/bundle[_-]}
        path = File.join 'man', "#{strip_version_from_url(path)}.ronn"
        'bundler/bundler'
      else
    ...
    link_to('Edit this document on GitHub', url) + ' if you caught an error or noticed something was missing.' + " (url is #{url} for debugging)"
  end
